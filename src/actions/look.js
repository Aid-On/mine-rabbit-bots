export function register(bot, commandHandlers, ctx) {
  commandHandlers.set('look', ({ args, sender }) => {
    ...
    const lookDir = async (kind) => {
      ...
      let dir = null;
      if (kind === 'front') dir = front;
      else if (kind === 'back') dir = back;
      else if (kind === 'left') dir = left;
      else if (kind === 'right') dir = right;
      if (dir) {
        const target = eye.plus({ x: dir.x * 6, y: dir.y * 6, z: dir.z * 6 });
    ...
    (async () => {
      try {
        ...
        if (args.length >= 3 && !isNaN(Number(args[0])) && !isNaN(Number(args[1])) && !isNaN(Number(args[2]))) {
          ...
          await bot.lookAt({ x: x + 0.5, y: y + 0.5, z: z + 0.5 }, true);
          return;
        }

        const name = args[0];
        const pl = bot.players[name];
        if (pl?.entity?.position) {
          const p = pl.entity.position;
          await bot.lookAt({ x: p.x, y: p.y + 1.6, z: p.z }, true);
          return;
        }
        say('対象が見つかりません');
      } catch (e) {
