export function register(bot, commandHandlers, ctx) {
  ...
  commandHandlers.set('build', ({ args, sender }) => {
    ...
    if (!blockName) { bot.chat(sender ? `@${sender} 使用方法: build <blockName> [front|back|left|right|up|down|near]` : 'usage: build <blockName> [dir]'); return; }
    const item = bot.inventory.items().find((i) => i.name === blockName);
    if (!item) { bot.chat(sender ? `@${sender} インベントリに ${blockName} がありません` : `no ${blockName}`); return; }
    ...
    let targetPos = null; let ref = null;
    if (dirArg === 'near') {
      const found = pickNear(); if (found) { targetPos = found.tp; ref = found.r; }
    } else {
    ...
    if (!targetPos) { bot.chat(sender ? `@${sender} 近くに設置できる場所が見つかりません` : 'no place'); return; }
    const targetBlock = bot.blockAt(targetPos);
    if (!isAirLike(targetBlock)) { const found = pickNear(); if (found) { targetPos = found.tp; ref = found.r; } }
    if (!ref) ref = findRef(targetPos);
    if (!ref) { bot.chat(sender ? `@${sender} 参照ブロックが見つからず設置できません` : 'no ref'); return; }

    (async () => {
      await bot.equip(item, 'hand');
      bot.setControlState('sneak', true);
      try { await bot.placeBlock(ref.refBlock, ref.face); }
      catch (err) { ctx.log?.(`設置できませんでした: ${err.message}`); bot.chat(sender ? `@${sender} 設置失敗: ${err.message}` : `fail: ${err.message}`); }
      finally { setTimeout(() => bot.setControlState('sneak', false), 300); }
    })();
  });
}
